...
data = pd.read_csv(os.path.join('Final forecasts', 'Top1_All_forecasts_{0}.csv'.format(dataset)), index_col=0)
...
errors.drop(columns='Real price')
...
for model in errors.columns:
    errors.loc[:, model] = mean_daily_errors(forecast=data[model].to_numpy().reshape(-1, 24),
                                             price=data['Real price'].to_numpy().reshape(-1, 24))
...
for model1 in pvals.index:
    for model2 in pvals.columns:
        pvals.loc[model1, model2] = gwtest(loss1=errors.loc[:, model1].values, loss2=errors.loc[:, model2].values)
...
# plt.savefig(f'GW_{dataset}.png', dpi=300)
plt.savefig('GW_{0}.eps'.fomat(dataset))
plt.show()

...
def MAE(p_real, p_pred):
    """ Computes the mean absolute error (MAE) between predicted and observed values:

    .. math::
        \\mathrm{MAE} = \\frac{1}{N}\\sum_{i=1}^N \\bigl|p_\\mathrm{real}[i]-p_\\mathrm{pred}[i]\\bigr|    

    The `p_real` and the `p_pred` tables can either be of shape
    ...
    ----------
        p_real : numpy.ndarray | pandas.DataFrame | pandas.Series
            Tabular data containing the observed prices.
        p_pred : numpy.ndarray | pandas.DataFrame | pandas.Series
            Tabular data containing the predicted prices.
    ...
    # calculate the absolute error at every time point
    ae = np.abs(p_real - p_pred)
...
if __name__ == "__main__":
    ...
    # Evaluate the MAE when real day-ahead prices and forecast are both 1-dimensional (long) dataframes
    print('MAE(p_pred=fc_DNN_ensemble, p_real=real_price): {0}'.
          format(MAE(p_pred=fc_DNN_ensemble, p_real=real_price)))
    # MAE(p_pred=fc_DNN_ensemble, p_real=real_price): 1.6833803823261926

    # Evaluate the MAE when real day-ahead price and forecasts are both pandas series
    ...
    # MAE(p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"], p_real=real_price.loc[:, "Price"]): 1.6833803823261926

    # Evaluate the MAE when real day-ahead price and forecasts are both 1-dimensional (long) numpy arrays
    ...
    # MAE(p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"].values,
    #     p_real=real_price.loc[:, "Price"].values): 1.6833803823261926

    # Evaluate the MAE when real day-ahead price and forecasts are both 2-dimensional (wide) DataFrames
    ...
    # MAE(p_pred=fc_DNN_ensemble_2D, p_real=real_price_2D): 1.6833803823261926
    ...
    # MAE(p_pred=fc_DNN_ensemble_2D.values.squeeze(), p_real=real_price_2D.values.squeeze()): 1.6833803823261926

...
def MAPE(p_real, p_pred, noNaN=False):
    """ Computes the mean absolute percentage error (MAPE) between predicted and observed values:
    ...
    ----------
        ...
        noNaN : boolean
            Some of the individual values of absolute percentage error can be Nan.
            This argument is used to remove the NaN values from mean calculus.
    Returns
    -------
        float
    ...
    # Calculate the absolute percentage error at every time point
    ape = np.abs(p_real - p_pred) / np.abs(p_real)

    # Eliminate NaN values if requested and averaging
    if noNaN:
        ape = ape[np.isfinite(ape)]

    return np.mean(a=ape)
...
if __name__ == '__main__':
    ...
    # Evaluate the MAPE when real day-ahead prices and forecast are both 1-dimensional (long) dataframes
    print('MAPE(p_pred=fc_DNN_ensemble, p_real=real_price) * 100: {0}'.
          format(MAPE(p_pred=fc_DNN_ensemble, p_real=real_price) * 100))
    # MAPE(p_pred=fc_DNN_ensemble, p_real=real_price) * 100: 5.38352491508943

    # Evaluate the MAPE when real day-ahead price and forecasts are both pandas series
    print('MAPE(p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"], p_real=real_price.loc[:, "Price"]) * 100: {0}'.
          format(MAPE(p_pred=fc_DNN_ensemble.loc[:, 'DNN Ensemble'], p_real=real_price.loc[:, 'Price']) * 100))
    # MAPE(p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"], p_real=real_price.loc[:, "Price"]) * 100: 5.38352491508943

    # Evaluate the MAPE when real day-ahead price and forecasts are both 1-dimensional (long) numpy arrays
    print('MAPE(p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"].values, '
          'p_real=real_price.loc[:, "Price"].values) * 100: {0}'.
          format(MAPE(p_pred=fc_DNN_ensemble.loc[:, 'DNN Ensemble'].values,
                      p_real=real_price.loc[:, 'Price'].values) * 100))
    # MAPE(p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"].values,
    #      p_real=real_price.loc[:, "Price"].values) * 100: 5.38352491508943

    # Evaluate the MAPE when real day-ahead price and forecasts are both 2-dimensional (wide) DataFrames
    # (rows: n_days, columns: n_prices/n_day)
    print('MAPE(p_pred=fc_DNN_ensemble_2D, p_real=real_price_2D) * 100: {0}'.
          format(MAPE(p_pred=fc_DNN_ensemble_2D, p_real=real_price_2D) * 100))
    # MAPE(p_pred=fc_DNN_ensemble_2D, p_real=real_price_2D) * 100: 5.3835249150894295

    # Evaluate the MAE when real day-ahead price and forecasts are both 2-dimensional (wide) numpy arrays
    # (rows: n_days, columns: n_prices/n_day)
    print('MAPE(p_pred=fc_DNN_ensemble_2D.values.squeeze(), p_real=real_price_2D.values.squeeze()) * 100: {0}'.
          format(MAPE(p_pred=fc_DNN_ensemble_2D.values.squeeze(), p_real=real_price_2D.values.squeeze()) * 100))
    # MAPE(p_pred=fc_DNN_ensemble_2D.values.squeeze(), p_real=real_price_2D.values.squeeze()) * 100: 5.3835249150894295

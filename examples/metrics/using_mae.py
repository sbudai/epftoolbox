...
# Evaluating MAE when real day-ahead price and forecasts are of different object type
# (numpy.ndarray and pandas.DataFrame)
try:
    print('MAE: {0}'.format(MAE(p_pred=fc_DNN_ensemble.values, p_real=real_price)))
...
try:
    print('MAE: {0}'.format(MAE(p_pred=fc_DNN_ensemble, p_real=real_price.loc[:, 'Price'])))
except TypeError as e:
    print("TypeError:", e)

# Evaluating MAE when real day-ahead price and forecasts are both numpy arrays but of different size
try:
    print('MAE: {0}'.format(MAE(p_pred=fc_DNN_ensemble.values, p_real=real_price.values[1:])))
except ValueError as e:
    print("ValueError:", e)

# Evaluating MAE when real day-ahead price and forecasts are both DataFrames but of different size
try:
    print('MAE: {0}'.format(MAE(p_pred=fc_DNN_ensemble.iloc[1:, :], p_real=real_price)))
except ValueError as e:
    print("ValueError:", e)

...
# According to the paper, the sMAPE of the 'DNN Ensemble' day-ahead price forecast for the NP market is 4.85%.
# Let's test the metric for different conditions

# Evaluating sMAPE when real day-ahead price and forecasts are both dataframes
print('sMAPE(p_real=real_price, p_pred=fc_DNN_ensemble): {0:5.2f}%'.
      format(sMAPE(p_real=real_price, p_pred=fc_DNN_ensemble) * 100))

# Evaluating sMAPE when real day-ahead price and forecasts are both pandas Series
print('sMAPE(p_real=real_price.loc[:, "Price"], p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"]): {0:5.2f}%'.
      format(sMAPE(p_real=real_price.loc[:, 'Price'], p_pred=fc_DNN_ensemble.loc[:, 'DNN Ensemble']) * 100))

# Evaluating sMAPE when real day-ahead price and forecasts are both 1-dimensional numpy arrays
print('sMAPE(p_real=real_price.loc[:, "Price"].values,'
      ' p_pred=fc_DNN_ensemble.loc[:, "DNN Ensemble"].values): {0:5.2f}%'.
      format(sMAPE(p_real=real_price.loc[:, 'Price'].values,
                   p_pred=fc_DNN_ensemble.loc[:, 'DNN Ensemble'].values) * 100))

# Evaluating sMAPE when real day-ahead price and forecasts are both
# 2-dimensional (rows: n_days, columns: n_prices/n_day) DataFrames
print('sMAPE(p_real=real_price_2D, p_pred=fc_DNN_ensemble_2D): {0:5.2f}%'.
      format(sMAPE(p_real=real_price_2D, p_pred=fc_DNN_ensemble_2D) * 100))

# Evaluating sMAPE when real day-ahead price and forecasts are both
# 2-dimensional (rows: n_days, columns: n_prices/n_day) numpy arrays
print('sMAPE(p_real=real_price_2D.values.squeeze(), p_pred=fc_DNN_ensemble_2D.values.squeeze()): {0:5.2f}%'.
      format(sMAPE(p_real=real_price_2D.values.squeeze(), p_pred=fc_DNN_ensemble_2D.values.squeeze()) * 100))


# We can also test situations where the sMAPE will display errors

# Evaluating sMAPE when real day-ahead price and forecasts are of different object type
# (numpy.ndarray and pandas.DataFrame)
try:
    print('sMAPE: {0:5.2f}%'.format(sMAPE(p_real=real_price, p_pred=fc_DNN_ensemble.values) * 100))
except TypeError as e:
    print("TypeError:", e)

# Evaluating sMAPE when real day-ahead price and forecasts are of the different object type
# (pandas.Series and pandas.DataFrame)
try:
    print('sMAPE: {0:5.2f}%'.format(sMAPE(p_real=real_price.loc[:, 'Price'], p_pred=fc_DNN_ensemble) * 100))
except TypeError as e:
    print("TypeError:", e)

# Evaluating sMAPE when real day-ahead price and forecasts are both numpy arrays but of different size
try:
    print('sMAPE: {0:5.2f}%'.format(sMAPE(p_real=real_price.values[1:], p_pred=fc_DNN_ensemble.values) * 100))
except ValueError as e:
    print("ValueError:", e)

# Evaluating sMAPE when real day-ahead price and forecasts are both DataFrames but of different size
try:
    print('sMAPE: {0}'.format(sMAPE(p_real=real_price, p_pred=fc_DNN_ensemble.iloc[1:, :])))
except ValueError as e:
    print("ValueError:", e)
